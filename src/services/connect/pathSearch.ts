...
<<<<<<< HEAD
  ...
  async function getNeighbors(nodeId: string): Promise<PersonEdge[]> {
    const m = neighborsMemo.get(nodeId);
    if (m) return m;
    const edges = await getPrunedNeighbors(nodeId);
    const capped = edges
      .sort((a, b) => b.confidence - a.confidence)
      .slice(0, MAX_NEIGHBORS);
    neighborsMemo.set(nodeId, capped);
    return capped;
  }
...
<<<<<<< HEAD
    ...
    await store.setPath(personAId, personBId, path);
    return path;
  } catch (e) {
